...
class DeclareChecker:
    ...
    def check_constraints(self, with_aggregates=False):
        res = {
            # First we check the object-level constraints
            self.config.OBJECT: self.check_object_level_constraints(with_aggregates=with_aggregates)
            # Then we check the multi-object constraints
            , self.config.MULTI_OBJECT: self.check_multi_object_constraints(with_aggregates=with_aggregates)
            # Then we check the activity-level constraints
            , self.config.ACTIVITY: self.check_activity_level_constraints(with_aggregates=with_aggregates)
            # Then we check the resource constraints
            , self.config.RESOURCE: self.check_resource_level_constraints(with_aggregates=with_aggregates)
    ...
    def get_constraint_strings(self, level: str):
        ...
        for idx, row in self.constraints[self.constraints[self.config.LEVEL] == level].iterrows():
            ...
            constraint_strings[const_str] = row[self.config.TEMPLATE]
        return constraint_strings

    def check_object_level_constraints(self, with_aggregates=False):
        filtered_traces = self.get_filtered_traces(self.log, parsed_tasks=self.activities_to_parsed,
                                                   with_loops=self.config.LOOPS)
        res = {}
        # aggregate results and provide fequencies
        agg_res = {}
    ...
    def check_multi_object_constraints(self, with_aggregates=False):
        filtered_traces = self.get_filtered_traces(self.log, parsed_tasks=self.activities_to_parsed,
                                                   with_loops=self.config.LOOPS)
        d4py = Declare(self.config)
    ...
    def check_activity_level_constraints(self, with_aggregates=False):
        filtered_traces = self.get_filtered_traces(self.log, parsed_tasks=self.activities_to_parsed,
                                                   with_loops=self.config.LOOPS)
        d4py = Declare(self.config)
    ...
    def check_resource_level_constraints(self, with_aggregates=False):
        filtered_traces = self.get_filtered_traces(self.log, parsed_tasks=self.activities_to_parsed,
